...
function rateLabel(rates = {}) {
  const entries = Object.entries(rates).slice(0, 4);
  return entries.length ? entries.map(([key, value]) => `${escapeHtml(key)} ${escapeHtml(String(value))}`).join(' · ') : 'Rates personalizados';
}

function card(server) {
  const isNew = Date.now() - new Date(server.created_at).getTime() < 14 * 86400000;
  return `<article class="server-card" data-server-card data-game="${escapeHtml(server.game)}" data-type="${escapeHtml(server.server_type)}" data-region="${escapeHtml(server.region)}" data-language="${escapeHtml(server.language)}" data-mods="${server.has_mods}" data-propagators="${server.uses_propagators}" data-platforms="${escapeHtml(server.platforms?.join('|').toLowerCase() || '')}" data-cluster="${server.cluster_name ? 'true' : 'false'}">
    <div class="server-card-media">${server.banner_url ? `<img src="${escapeHtml(server.banner_url)}" alt="" loading="lazy">` : '<span>W.E.A.F / SERVER</span>'}<div class="server-badges">${server.is_featured ? '<b>Destacado</b>' : ''}${server.is_verified ? '<b>Verificado</b>' : ''}${isNew ? '<b>Nuevo</b>' : ''}</div></div>
...
export function render() {
  return `<section class="servers-hero"><div class="container"><p>Directorio comunitario</p><h1>Encuentra un servidor con tus reglas.</h1><div><span>ASE + ASA</span><span>Información directa</span><span>Sin rankings artificiales</span></div></div></section>
