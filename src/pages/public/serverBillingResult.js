...
export function render({ path }) {
  if (path === '/servers/cancel') {
    ...
    return `<section class="publish-shell container"><div class="publish-message"><p>${t('servers.result.canceled')}</p><h1>${t('servers.result.noCharge')}</h1><p>${t('servers.result.saved')}</p><a class="button button-primary" href="/servers/publish?listing_id=${escapeHtml(listingId)}" data-link>${t('servers.result.back')}</a><a class="button button-quiet" href="/servers/owners" data-link>${t('servers.result.plans')}</a></div></section>`;
  }
  return `<section class="publish-shell container"><div class="publish-message" data-checkout-result><p>${t('servers.result.received')}</p><h1>${t('servers.result.confirming')}</h1><p>${t('servers.result.webhook')}</p></div></section>`;
}
