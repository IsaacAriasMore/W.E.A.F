...
export function bind({ path, authService, navigate }) {
  ...
  root.addEventListener('change', (event) => {
    if (event.target.name !== 'publication_plan') return;
  ...
  root.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (result.error) showToast(result.error, 'error');
        ...
        let approval;
        try { approval = new URL(order.data?.url); } catch { approval = null; }
        const trusted = approval?.protocol === 'https:' && (approval.hostname === 'paypal.com' || approval.hostname.endsWith('.paypal.com'));
        if (order.error || !trusted) { showToast(order.error || t('marketplace.errors.paymentStart'), 'error'); navigate('/account/marketplace'); }
        else window.location.assign(approval.href);
