...
do $$
begin
  -- Storage upserts require a SELECT policy in addition to INSERT and UPDATE.
  -- Public bucket delivery remains public; this policy only permits authenticated
  -- users to read their own object through the Storage API.
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'weaf_avatar_select_own') then
    create policy weaf_avatar_select_own on storage.objects for select to authenticated
