-- Make public server publishing understandable while preserving the existing schema.

create or replace function private.server_listing_slug(p_title text)
returns text
language plpgsql
volatile
set search_path = ''
...
begin
  base_slug := trim(both '-' from regexp_replace(
    translate(lower(trim(p_title)), 'áéíóúüñ', 'aeiouun'),
    '[^a-z0-9]+', '-', 'g'
  ));
  base_slug := left(coalesce(nullif(base_slug, ''), 'servidor'), 72);
  candidate := base_slug;
  while exists (select 1 from public.server_listings where slug = candidate) loop
    candidate := left(base_slug, 65) || '-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 6);
  end loop;
  return candidate;
end;
$$;

revoke all on function private.server_listing_slug(text) from public, anon, authenticated;
...
begin
  ...
  if p_listing_id is not null then
    ...
    generated_slug := private.server_listing_slug(p_payload->>'title');
