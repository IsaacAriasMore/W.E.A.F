-- Tracking is called only by the Edge Function's admin client. Authorization is
-- enforced by the function ACL below; API-key claims are not reliable session
-- settings when Supabase uses the newer secret-key model.
